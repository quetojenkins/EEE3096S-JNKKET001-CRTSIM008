...
void TIM16_IRQHandler(void)
{
	...
	uint8_t read = read_from_address(pattern_no);
	if (read == patterns[pattern_no]){
		write_to_LEDS(read);
	}else{write_to_LEDS(0b00000001);}
...
// function to write bits to the LEDS
void write_to_LEDS(uint8_t number){
	uint8_t bits[8];
